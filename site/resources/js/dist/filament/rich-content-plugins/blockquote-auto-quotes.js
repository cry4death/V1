...
// resources/js/filament/rich-content-plugins/blockquote-auto-quotes.js
...
var blockquote_auto_quotes_default = index_default.extend({
  addCommands() {
    return {
      ...this.parent?.(),
      toggleBlockquote: () => ({ chain, editor }) => {
          ...
          for (let i = 0; i < bq.childCount; i++) {
            ...
            if (child.type.name === "paragraph" && child.textContent.trim()) {
              ...
              if (wrapped !== plain) {
                ...
                let marks = [];
                if (child.childCount === 1 && child.firstChild?.isText) {
...
export {
  blockquote_auto_quotes_default as default
};
